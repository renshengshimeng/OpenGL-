...
//#include <math.h>
/*
 GLMatrixStack 变化管线使用矩阵
 GLMatrixStack
    构造函数允许指定堆栈的最大深度、默认的堆栈深度为64
    .这个矩阵堆在初始化时已经在堆栈中包含了单元矩阵
...
void SetupRC() {
    ...
    /*
     ...
     参数3：纹理坐标（可选）
     // 复制顶点坐标
     void GLBatch::CopyVertexData3f(GLFloat *vNormas);
     // 结束，表示已经完成数据复制工作
    ...
    GLfloat radius = 3.0f;
    // 从0度~360度，以0.e弧度为步长
    for(GLfloat angle = 0.0f;angle <= M3D_2PI; angle += 0.3) {
        // 或许圆形的顶点x,y
...
void KeyPressFunc(unsigned char key, int x, int y) {
    ...
    switch (nStep) {
        ...
        case 1:
            ...
            break;
        case 2:
            glutSetWindowTitle("GL_LINES_STRIP");
            break;
        case 3:
